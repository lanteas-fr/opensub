				...
				if(isset($field['type']))
				{
					switch($field["type"])
					{
                        ...
                        if($field['massupdate'] == 1){
                                ...
                                $even = !$even; $newhtml .= $this->addUrlBox($displayname,  $field["name"]);
...
EOQ;
	...
	public function addUrlBox($displayname, $field){
        $displayname = addslashes($displayname);
        $html = <<<EOQ
    <td scope="row" width="20%">$displayname</td>
    <td class='dataField' width="30%"><input type="url" name='$field' size="18" id='{$field}' maxlength='255' value=""></td>
